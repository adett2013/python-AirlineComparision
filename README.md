<div align="center">

<!-- Plane animation banner -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:1a73e8,100:00b4d8&height=180&section=header&text=Airline%20Chooser%20🛫&fontSize=52&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Lufthansa%20VS%20Eurowings%20—%20Which%20airline%20suits%20you%3F&descAlignY=58&descSize=18" width="100%"/>

</div>

---

<div align="center">

![Python](https://img.shields.io/badge/Python-3.x-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Status](https://img.shields.io/badge/Status-Active-brightgreen?style=for-the-badge)
![License](https://img.shields.io/badge/License-MIT-yellow?style=for-the-badge)
![Aviation](https://img.shields.io/badge/Aviation-Lover-blue?style=for-the-badge&logo=airplanemodeactive)

</div>

---

## 🌐 About

**Airline Chooser** is a Python-powered interactive CLI tool that helps you decide between **Lufthansa ✈️** and **Eurowings 🛫** based on your personal travel preferences.

Answer a series of simple **yes/no** questions and the system will recommend the best airline for your next journey — whether it's a quick European getaway or a long-haul adventure!

---

## 🚀 How It Works

```
python main.py
```

The program asks you **10 questions** about your travel preferences. Based on your answers, it recommends either **Lufthansa** or **Eurowings**.

---

## 🗺️ Questions Asked

| # | Question | Favours |
|---|----------|---------|
| 1 | Is an included snack and drink on board important to you? | ✈️ Lufthansa |
| 2 | Do you have a high busget for your ticket? | ✈️ Lufthansa |
| 3 | Do you need checked bags? | ✈️ Lufthansa |
| 4 | Is flexibility important to you for rebooking? | ✈️ Lufthansa |
| 5 | Are you travelling with kids (family)? | 🛫 Eurowings |
| 6 | Are you travelling to holiday-places like mallorca? | 🛫 Eurowings |
| 7 | Do you want to fly from Hamburg Airport directly? | 🛫 Eurowings |
| 8 | Do you need priority-services like included airport-checkin? | ✈️ Lufthansa |
| 9 | Are you flying long-haul? | ✈️ Lufthansa |
| 10 | Is seat-comfort and legroom important to you? | ✈️ Lufthansa |
| 🔁 | *(Tiebreaker)* Is this trip for business? | ✈️ Lufthansa |

> You can answer each question with **yes** (or **y**) and **no** (or **n**).

---

## ✈️ Airline Profiles

<table>
<tr>
<td align="center" width="50%">

### ✈️ Lufthansa

**Best for:**
- Business travellers
- Long-haul flights
- Comfort-focused passengers
- Checked baggage needs
- Rebooking flexibility
- Priority services

</td>
<td align="center" width="50%">

### 🛫 Eurowings

**Best for:**
- Budget-conscious travellers
- Family holidays
- Popular European destinations (e.g. Mallorca)
- Hamburg Airport departures
- Short-haul leisure flights

</td>
</tr>
</table>

---

## 🛠️ Getting Started

### Prerequisites

- Python 3.x installed on your machine

### Installation & Run

```bash
# Clone the repository
git clone https://github.com/adett2013/python-AirlineComparision.git

# Navigate to the project folder
cd python-AirlineComparision

# Run the script
python main.py
```

---

## 📋 Example Output

```
Welcome to the Airline Chooser 🛫
In the moment you can compare Lufthansa VS Eurowings
The system will ask you questions. You can answer with yes (or y) and no (or n)

Is an included snack and drink on board important to you? yes
Do you have a high budget for your ticket? yes
...

-YOUR RESULTS-
Based on your answers, Lufthansa is a better choice for you.
```

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00b4d8,100:1a73e8&height=120&section=footer&animation=fadeIn" width="100%"/>

*Made with ❤️ for aviation lovers* ✈️🌍🛫

</div>